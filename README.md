...
一个本地运行的科研论文分析 Agent 工作台，用于把“研究方向”转成可复盘的论文分析流程。

## 1. 这个项目是什么

Research-Agent 是一个本地科研论文分析工作台。它包含 FastAPI 后端和 Streamlit 中文前端，用户可以通过页面或 API 输入一个研究方向，然后让系统执行一组固定的研究步骤。

它不是普通聊天机器人。普通聊天机器人通常只生成一段回答，而这个项目会调用后端工具，记录搜索结果、workflow run、RAG trace 和评估反馈。

它也不是单纯的论文搜索工具。项目把论文搜索、流程执行、RAG 检索、报告生成和评估记录串在一起，方便后续复盘每一步做了什么、用了哪些证据、哪里需要改进。
...
- RAG 问答如果只返回答案，很难知道答案来自哪些论文片段。
- 检索效果好不好，通常缺少 trace 和反馈记录来复盘。

Research-Agent 的目标是提供一个本地可运行的流程：从研究方向开始，逐步得到论文记录、RAG 证据、知识树、创新点和 Markdown 报告。
...
- FastAPI 后端：提供 API、Agent 编排、workflow、RAG 和持久化能力。
...
- RAG v1 能力边界：[docs/rag_v1_quality_boundary.md](docs/rag_v1_quality_boundary.md)
