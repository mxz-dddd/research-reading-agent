...
当前 RAG 是支撑科研阅读的本地检索能力：已 ingest 论文文本会被切分为 chunks，保存到 SQLite `rag_chunks` 表。keyword 模式保留 RAG v1 的本地关键词 / token overlap 检索；hybrid 模式提供 RAG v2 的 contextual chunking、本地 hash dense retrieval、sparse retrieval、RRF fusion 和 deterministic rerank。RAG v2 不调用 OpenAI embedding，不接 Qdrant，不引入外部 reranker，回答也先采用模板化保守生成。现在 workflow 会在 ingest 后按 `index_rag` 开关自动建立 RAG 索引，形成“读论文 -> 建索引 -> 后续检索问答”的主闭环。RAG evidence 会返回 `matched_terms` 和 `score_reason`，并在 no evidence 时明确拒绝基于文档回答。每次 RAG search / answer 默认保存 Evidence Trace，记录 query、evidence、answer 和 no_evidence 状态；人工 feedback 会基于这些 trace 形成轻量 evaluation summary，evidence-level feedback 会进一步计算 Recall@K、MRR 和 nDCG@5。
...
RAG 工具会复用 `RagService`，完成论文文本索引、SQLite chunk 检索、模板化证据回答和 evidence trace 记录。workflow 也复用同一个 `RagService.index_paper_for_rag()`，在 ingest 后为成功深入阅读的论文建立索引。它支持 RAG v1 keyword 模式和 RAG v2 本地轻量 hybrid 模式；当前不包含 Qdrant、真实 sentence-transformers embedding、外部 reranker 或 LLM 证据融合。
...
- 不支持长期 Memory。
- 不支持复杂任务规划。
- RAG v1 只做关键词 / token overlap 检索；RAG v2 是本地轻量 hybrid retrieval，不是 Qdrant / 生产级向量库。质量边界记录在 `docs/rag_v1_quality_boundary.md` 和 `docs/rag_v2_context_pack.md`。
